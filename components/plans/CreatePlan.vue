  ...
  <div class="admin p-4">
    <b-row>
      <b-modal
        class="backdropblur"
        size="xl"
        id="plans-create-plan"
        title="Creación de Plans"
      >
        <b-row>
          <!-- Name of the plan-->
          <b-col cols="12">
          ...
          <b-col cols="6">
            <b-form-group
              ...
              label-for="nombre-plan-1"
            >
              <b-input-group prepend="¿Activo?">
                <b-form-select
                  id="active-1"
                  v-model="dataPlan.active"
                  :options="activesOption"
                  required
                ></b-form-select>
              </b-input-group>
              <b-form-invalid-feedback id="input-1-live-feedback">{{
                veeErrors.first("active-plan-1")
              }}</b-form-invalid-feedback>
            </b-form-group>
          </b-col>
        </b-row>
        <div slot="modal-footer">
          <b-btn
            class="float-right"
            variant="danger"
            @click="$bvModal.hide('plans-create-plan')"
          >
            Close
          </b-btn>
  ...
  methods: {
    ...
    async createPlan() {
      this.$validator.validateAll().then(async (result) => {
        if (result) {
          this.$axios
            .post("/api/plan", this.dataPlan)
            .then(() => {
              this.$swal.fire(
                "Creación exitosa",
                "Plan creado de manera correcta en el sistema",
                "success"
              );
              this.resetForm();
